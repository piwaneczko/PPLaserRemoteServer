...
enum XmlConfigFlag
{
    XmlConfigReadFlag = 0x01,
    XmlConfigWriteFlag = 0x02,
    XmlConfigReadWriteFlag = 0x03
};

template<typename T, XmlConfigFlag f = XmlConfigReadFlag> class XmlConfigValue : public XmlConfigElement
{
private:
    T value;
...
private:
    ...
    void InitValue(xml_attribute &attr, bool get, int32_t &value) {
        if (get) value = attr.as_int(value);
        else attr = value;
    }
    ...
    void InitValue(xml_attribute &attr, bool get, string &value) {
        if (get) value = attr.as_string(value.c_str());
        else attr = value.c_str();
    }
    ...
    void InitValue(xml_attribute &attr, bool get, uint32_t &value) {
        if (get) value = attr.as_uint(value);
        else attr = value;
    }
...
public:
    ...
    /**
     * \fn  template < typename = typename XmlConfigValue::enable_if< (f & XmlConfigWriteFlag) == XmlConfigWriteFlag >::type > void operator=(const T &value)
     *
...
template<typename K, typename T, XmlConfigFlag f = XmlConfigReadFlag> class XmlConfigMap : public XmlConfigGroup, public map<K, XmlConfigValue<T, f>> {
protected:
    /**
     * Insert pair to map
     */
